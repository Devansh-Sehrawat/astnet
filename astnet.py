...
class Gradient_Loss(nn.Module):
    def __init__(self, channels):
        ...
        self.filter_x = torch.stack((neg, pos)).unsqueeze(0).permute(3, 2, 0, 1).to(device=config.GPUS[0])
        self.filter_y = torch.stack((pos.unsqueeze(0), neg.unsqueeze(0))).permute(3, 2, 0, 1).to(device=config.GPUS[0])
...
def eval_(model_file, config, suffix):
    ...
    paths = dict(log_dir="%s/%s" % ('./log', config.DATASET.DATASET),
                 ckpt_dir="%s/%s" % ('./ckpt', config.DATASET.DATASET))

    model_save_path = os.path.join(paths["ckpt_dir"], config.DATASET.DATASET + config.MODEL.NAME)

    model = models.get_net(config).to(device=config.GPUS[0]).eval()
    ...
    # logger.info('Epoch: '.format(model_file))
...
if __name__ == '__main__':
    ...
    print(eval_(args.model_file, config, 'eval'))
